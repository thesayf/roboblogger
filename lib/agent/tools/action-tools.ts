  ...
  return [
    betaZodTool({
      ...
      inputSchema: z.object({
        ...
        tone: z.string().max(500).optional().describe('Writing tone — keep concise (e.g., "professional", "casual but authoritative", "technical")'),
        length: z.string().max(200).optional().describe('Post length (e.g., short, medium, long, "2000-3000 words")'),
        ...
        additionalRequirements: z.string().max(1000).optional().describe('Additional instructions for content generation'),
        imageContext: z.string().max(1000).optional().describe('Image style description for generated images (e.g., "Modern minimalist with blue corporate tones", "Warm watercolor illustrations")'),
    ...
    betaZodTool({
      ...
      inputSchema: z.object({
        topics: z.array(z.object({
          topic: z.string().describe('The blog topic/title'),
          audience: z.string().optional().describe('Target audience for this post'),
          tone: z.string().max(500).optional().describe('Writing tone — keep concise (e.g., "professional", "casual but authoritative")'),
          length: z.string().max(200).optional().describe('Post length (e.g., short, medium, long, "2000-3000 words")'),
          ...
          scheduledAt: z.string().optional().describe('ISO date string for scheduled generation'),
          additionalRequirements: z.string().max(1000).optional().describe('Additional instructions for content generation'),
          imageContext: z.string().max(1000).optional().describe('Image style description for generated images'),
    ...
    betaZodTool({
      ...
      inputSchema: z.object({
        ...
        tags: z.array(z.string()).optional(),
        additionalRequirements: z.string().max(1000).optional().describe('Specific writing instructions: key points, angle, structure'),
        imageContext: z.string().max(1000).optional().describe('Image style description for generated images (e.g., "Modern minimalist with blue corporate tones")'),
        referenceImages: z.array(z.string()).optional().describe('URLs of reference images from the media library to guide image generation style'),
        internalLinks: z.array(z.object({
          postId: z.string().describe('Blog post ID to link to'),
          slug: z.string().describe('Post slug for the link URL'),
